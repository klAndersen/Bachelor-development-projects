...
public class Kontroll {
	...
	private  Kontroll() {	} //tom konstruktør, satt som private så ingen får tilgang til den		

	/**
	 * Fikk tipset av Knut W. Hansson om å bruke Singleton.
	 * Singleton sørger for at kun et objekt av klassen det opprettes i er tilgjengelig.
	 * Kilde:
	 ...
	 * objektets verdi null, og hver gang programmet kaller på den, returneres det samme 
	 * objektet. Synchronized forhindrer at to klasser kaller på metoden samtidig.
	 */
	public static synchronized Kontroll getKontrollIinstans(String bruker, String pwd) {
		if (kontrollIinstans == null) { //hva er kontrollinstans verdi?
			try {
				...
				db =  getDatabase(bruker,pwd); //forsøk å koble til databasen
				HareTabell = db.hentHare(); //fyll ArrayList med Hare(r)
				GaupeTabell = db.hentGaupe(); //fyll ArrayList med Gaupe(r)
				lagMessageDialog("Du er nå tilkoblet databasen, og kan starte registrering.","Tilkobling suksessfull",1);
			} catch (NullPointerException e) { //feil brukernavn/passord, så finner ikke databasen
				lagMessageDialog("Grunnet tilkobling til database feilet, vil data ikke bli lagret.\nSjekk at brukernavn/passord er korrekt.",
						"Tilkobling",0);
				kontrollIinstans = null; //nullstiller instans, slik at nytt pålogginsforsøk er mulig
			} catch (Exception e) {
	...
	public static Database getDatabase(String bruker, String pwd) {
		try {			
			...
			lagMessageDialog("Får ikke forbindelse med databasen.","Tilkobling", 0);
		} // try/catch
		return null;
	} //getDatabase
	...
	public void lukkTilkobling() {
		try {
			...
			lagMessageDialog("Klarte ikke å lukke forbindelse.","Lukking", 0);
		} // try/catch
	} //lukkTilkobling
	...
	public static boolean sjekkId(String id, char type) { 
		...
		try {
			...
			lagMessageDialog("Databasetilkobling er ikke aktiv.\nDu kreves en aktiv tilkobling for å kunne søke på ID.", tittel, 0);
		} catch (Exception e) { //uventet feil
			lagMessageDialog("En feil oppsto under søk:\n"+ e, tittel, 0);
		} // try/catch
		return sjekk;
	} //sjekkId
